backend_args = None
class_name = ('smokestack', )
data_root = '../data/Mission1/'
...
default_hooks = dict(
    ...
    visualization=dict(
        draw=True, interval=2, show=False, type='DetVisualizationHook'))
default_scope = 'mmdet'
env_cfg = dict(
...
max_epochs = 50
metainfo = dict(classes=('smokestack', ))
model = dict(
    as_two_stage=True,
    backbone=dict(
        attn_drop_rate=0.0,
    ...
    test_cfg=dict(max_per_img=100, score_thr=0.25),
    train_cfg=dict(
        assigner=dict(
            match_costs=[
...
test_evaluator = dict(
    ann_file='../data/Mission1/annotations/val.json',
    backend_args=None,
    format_only=False,
    metric='bbox',
    type='CocoMetric')
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        backend_args=None,
        data_prefix=dict(img='Training/images/'),
        data_root='../data/Mission1/',
